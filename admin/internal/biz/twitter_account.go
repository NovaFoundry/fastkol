...
type TwitterAccountRepo interface {
	...
	List(context.Context, int, int, string, int64, string, string, string, string) ([]*TwitterAccount, int64, error)
...
// List 列出所有Twitter账号
func (uc *TwitterAccountUsecase) List(ctx context.Context, pageSize, pageNum int, status string, id int64, username, email string, sortField, sortOrder string) ([]*TwitterAccount, int64, error) {
	// 设置默认值
	if pageSize <= 0 {
		pageSize = 20 // 默认每页20条
	}
	...
	uc.log.WithContext(ctx).Infof("List TwitterAccounts: pageSize=%v, pageNum=%v, status=%v, id=%v, username=%v, email=%v, sortField=%v, sortOrder=%v",
