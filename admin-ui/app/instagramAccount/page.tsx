...
import api, { InstagramAccount, CreateAccountRequest } from '../services/instagramApi';
...
export default function Home() {
  ...
  const statusMap: Record<InstagramAccount['status'], string> = {
    normal: '正常',
    login_expired: '登录已失效',
    disabled: '已禁用',
    deprecated: '已废弃',
  ...
  const statusColor: Record<InstagramAccount['status'], string> = {
    normal: '#52c41a',
  ...
  const handleEdit = (record: InstagramAccount) => {
  ...
  const columns: ColumnsType<InstagramAccount> = [
    {
      title: 'ID',
      dataIndex: 'id',
      key: 'id',
    ...
    },
    {
      ...
      dataIndex: 'status',
      key: 'status',
      render: (status: InstagramAccount['status']) => {
      ...
      <div className="p-6">
        ...
        >
          ...
          >
            ...
            <Alert
              message="💡 提示"
              description="建议从 Instagram 请求中复制 headers，需要包含 cookie 和 x-csrftoken 字段"
              type="info"
              showIcon
              className="mb-4"
            />
